...
// 1lib-bloque-seguridad-peticion
// Valida el método POST y la sesión del usuario para prevenir ataques CSRF
...
try {
    // 1lib-bloque-autorizacion-moodle
    $courseid = required_param('courseid', PARAM_INT);
    require_sesskey();
    $context = context_course::instance($courseid);
    require_capability('moodle/course:manageactivities', $context);

    // 1lib-bloque-inicializacion-ia
    // Inyecta el cliente del core de IA para comunicación con LLM
    if (!class_exists('\local_ai_core\client')) {
        throw new Exception('El plugin local_ai_core no está instalado.');
    }
